...
class Manager:
    ...
    def get_author_schedule(self, author, weekday: int, is_overline: bool) -> List[db.Schedule]:
        if type(author) == str:
    ...
    def delete_schedule(self, group, weekday, is_overline, num, commit = True) -> None:
    ...
    def login_user(self, uid: int, group, state: int = 1, commit: bool = True) -> None:
        group = self.session.query(db.Group).filter(db.Group.group == group).first()
