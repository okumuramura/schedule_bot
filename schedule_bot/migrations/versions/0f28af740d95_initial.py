...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('lesson_type_id', sa.Integer, sa.ForeignKey('lesson_types.id'), nullable=True)
    )
    op.create_table(
        'active_users',
    ...
    op.bulk_insert(
        types_table,
        [
            {'id': i, 'type': lesson_type} for i, lesson_type in enumerate(lesson_types_str, start=1)
        ]
    )
